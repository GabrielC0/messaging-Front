...
console.log('⚠️  Note: L\'API peut prendre quelques secondes à se réveiller sur Render.com');
console.log('');
...
async function testGetConversations() {
  ...
  try {
    ...
    if (response.ok && result.data?.conversations) {
      console.log('✅ Conversations récupérées:', result.data.conversations.length);
      if (result.data.conversations.length > 0) {
        console.log('   Première conversation:', result.data.conversations[0].title || 'Sans titre');
      }
    } else {
...
async function testRestEndpoint() {
  ...
  try {
    const response = await fetch('https://messaging-platform-gfnp.onrender.com/health');
...
async function runTests() {
  ...
  await testCreateUser();
    console.log('📋 Résumé:');
  console.log('- L\'API est hébergée sur Render.com et peut prendre du temps à se réveiller');
  console.log('- La base de données PostgreSQL est en ligne');
  console.log('- Si les tests échouent au premier essai, réessayez dans quelques minutes');
