...
export function ApiTestPanel() {
  ...
  useEffect(() => {
    setTestResults({
      health: !healthLoading && !healthError && healthData?.healthCheck?.result === 'OK',
      users: !usersLoading && !usersError && Array.isArray(usersData?.users),
      conversations: !conversationsLoading && !conversationsError && Array.isArray(conversationsData?.conversations),
      websocket: socketConnected,
    });
  }, [
    healthLoading, healthError, healthData,
    usersLoading, usersError, usersData,
    conversationsLoading, conversationsError, conversationsData,
    socketConnected
  ]);
  ...
  const createTestConversation = async () => {
    ...
    try {
      const participants = usersData.users.slice(0, 2).map((user: User) => user.id);
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {conversationsData && (
            <div className="text-sm text-muted-foreground ml-4">
              {conversationsData.conversations.length} conversation(s) trouvée(s)
            </div>
          )}
          {conversationsError && (
            <div className="text-sm text-red-500 ml-4">
        ...
        <CardContent className="space-y-4">
          ...
          {testConversation && (
            <div className="p-3 bg-blue-50 rounded-lg">
              <h4 className="font-medium text-blue-800">Conversation créée :</h4>
              <p className="text-sm text-blue-700">
                {testConversation.title} - {testConversation.participants?.length} participants
              </p>
            </div>
          )}
        </CardContent>
